...
def ai_agents_router():
    d = request.get_json(slient = True)
    ...
    if "youtube" in cmd:
        q = cmd
        patterns = ["open youtube and search", "open youtube and play", "open youtube", "and play", "play", "on youtube"]
        for p in patterns:
            q = q.replace(p, "")
        q = q.strip()
        vid = get_vid(q)
        if vid:
            ...
            if recip_part:
                c = recip_part.replace(" at ", "@").replace(" dot ",".").replace(" dash ","-")
